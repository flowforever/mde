...
export interface ElectronBuildCommand {
  readonly args: readonly string[]
  readonly command: string
}

export const createElectronBuildCommand = (
  env: NodeJS.ProcessEnv = process.env
): ElectronBuildCommand => {
  const packageManagerExecPath = env.npm_execpath

  if (packageManagerExecPath?.includes('pnpm')) {
    return {
      command: process.execPath,
      args: [packageManagerExecPath, 'run', 'build']
    }
  }
...
export const buildElectronApp = async (): Promise<void> => {
  const { command, args } = createElectronBuildCommand()
