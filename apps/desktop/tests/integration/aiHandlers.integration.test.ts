...
describe("aiHandlers integration", () => {
  const registerHandlers = (workspacePath: string) => {
    ...
    const runPrompt: RunPrompt = ({ modelName, prompt, tool }) => {
      ...
      if (prompt.includes("application UI language pack")) {
  ...
  it("generates app language packs through the AI IPC handler", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "mde-ai-ipc-"));
    const { handlers } = registerHandlers(workspacePath);

    const result = (await handlers.get(AI_CHANNELS.generateAppLanguagePack)?.(
      {},
      "Spanish",
      [
  ...
  it("forwards summary refinement instructions through the AI IPC handler", async () => {
    ...
    await handlers.get(AI_CHANNELS.summarizeMarkdown)?.(
      {},
      "README.md",
      "# Readme",
      workspace.rootPath,
    );
    const refined = (await handlers.get(AI_CHANNELS.summarizeMarkdown)?.(
      {},
      "README.md",
      "# Readme",
      workspace.rootPath,
    ...
    await expect(
      readFile(join(workspacePath, refined.path), "utf8"),
  ...
  it("rejects stale AI requests when the active workspace changes", async () => {
    ...
    await expect(
      handlers.get(AI_CHANNELS.summarizeMarkdown)?.(
        {},
        "README.md",
        "# Readme",
        "/stale-workspace",
