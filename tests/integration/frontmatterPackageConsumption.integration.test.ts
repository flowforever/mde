...
const removedShimPaths = [
  join(process.cwd(), 'apps/desktop/src/shared/frontmatter.ts'),
  join(process.cwd(), 'apps/desktop/src/renderer/src/editor/frontmatter.ts')
]
...
describe('frontmatter package consumption', () => {
  it('keeps desktop code on the editor-core frontmatter package instead of local shims', async () => {
    ...
    for (const file of files) {
      ...
      if (
        /from ['"][^'"]*(?:shared\/frontmatter|editor\/frontmatter|\.\/frontmatter)['"]/u.test(
          contents
        )
      ) {
