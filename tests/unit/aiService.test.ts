...
const locateCommands =
  (paths: Partial<Record<"claude" | "codex", string>>): LocateCommand =>
  (tool) =>
...
describe("aiService", () => {
  ...
  it("generates app language packs as structured UI text entries", async () => {
    ...
    const result = await service.generateAppLanguagePack("Spanish", [
    ...
    expect(result).toEqual({
      entries: [
        ...
        { key: "workspace.openWorkspace", text: "Abrir espacio de trabajo" },
      ],
      language: "Spanish",
  ...
  it("regenerates cached translations when source Markdown changes", async () => {
    ...
    await service.translateMarkdown(
      workspacePath,
      "README.md",
      "# Readme",
      "English",
  ...
  it(
    "does not pass removed Codex approval flags when generating translations",
    async () => {
      const workspacePath = await mkdtemp(join(tmpdir(), "mde-ai-service-"));
  ...
  it(
    "does not pass removed Codex approval flags when generating summaries",
    async () => {
      const workspacePath = await mkdtemp(join(tmpdir(), "mde-ai-service-"));
      const binPath = await mkdtemp(join(tmpdir(), "mde-ai-codex-args-"));
      const fakeCodexPath = join(binPath, "codex");
