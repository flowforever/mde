...
打开 Automation Center 时，MDE 会先校验已缓存的发现结果，再决定是否跳过新的发现运行。仍然有效的缓存会继续使用，避免重复发现；没有本地文件路径的远程来源也会保留。

本地缓存如果对应文件已缺失、已移动到 `done/` 或 `archived/`、不再是 READY，或已经不符合当前 Automation Flow 允许读取的位置与匹配条件，就不会再阻止新的发现运行。此时 Automation Center 会丢弃这些过期结果并启动一次新的发现，Automation Run History 中会出现这次新的发现记录。
