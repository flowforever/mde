...
- 2026-05-03: Completed and released in `v1.4.5`.
- Completion summary: The editor top-right action bar now collapses when more than five actions are available. The collapsed state keeps Search, AI Translate, AI Summary, and Version History visible by priority, while the arrow expands line spacing and editor view controls. The editor view toggle now switches icons without exposing a pressed state.
- Verification notes: Local `npm run lint`, `npm run typecheck`, `npm run test:unit`, `npm run test:integration`, `npm run test:e2e`, `npm run docs:build`, `npm run build`, and `npm audit --audit-level=high` passed. Pre-push verification passed. Release workflow `25271838808` passed. User Manual deploy workflow `25271838812` passed. GitHub Release `v1.4.5` published with 12 artifacts.
